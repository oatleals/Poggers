 ...
 * Specifies mode to be focused on matching COLORS of cards
 */
public class ColorMode extends Mode
{

  public ColorMode(Player p)
  {
    super(p);
  }
 ...
 */
  public Boolean rightSpot(PoggersModel.MoveSubscriber userChoice)
  {
    if (userChoice == PoggersModel.MoveSubscriber.STACKTOMID)
    {
      ...
      System.out.println(middleCard.getCardColor() + " " + getPrevMiddleCard().getCardColor());
        if(middleCard.getCardColor().equals(getPrevMiddleCard().getCardColor())) 
        {
    ...
    else if(userChoice == PoggersModel.MoveSubscriber.STACKTOSIDE)
    {
      ...
      System.out.println(getPrevMiddleCard().getCardColor() + " " + getSideCard().getCardColor());
    
        if(getPrevMiddleCard().getCardColor().equals(getSideCard().getCardColor())) 
        {
